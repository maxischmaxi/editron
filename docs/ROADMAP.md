...
Nur ein Stub-Command („Marker folgen in einer späteren Version“,
`src/core/commands.rs`). Profi-Standard: Sequenz- und Clip-Marker mit
Farben/Notizen.

```text
Implementiere Marker in Editron — auf absolut professionellem Niveau, wie in Adobe Premiere Pro und DaVinci Resolve, sodass professionelle Video-Produzenten das Tool produktiv nutzen können.

Ausgangslage: Es gibt nur einen Stub-Command („Marker folgen in einer späteren Version“ in src/core/commands.rs).

Umfang:
1. Sequenz-Marker im Timeline-Modell (src/core/timeline.rs): Zeitpunkt (optional mit Dauer/Bereich), Name, Notiz, Farbe (8 Standardfarben wie Premiere). Clip-Marker analog am TimelineClip (Position in Medienzeit, wandern beim Trimmen korrekt mit). Persistenz im .etron-Format, Undo/Redo über den TimelineStore.
2. Commands + Shortcuts (alle Keymap-Presets): Marker setzen am Playhead (M), Marker setzen mit Dialog (Shift+M öffnet Name/Notiz/Farbe), zum nächsten/vorherigen Marker springen (Shift+Bild↑/↓ bzw. Premiere-Belegung), Marker löschen, alle Marker löschen. M während der Wiedergabe muss verzögerungsfrei funktionieren (Logging-Workflow).
3. Darstellung: Marker im Timeline-Lineal (farbige Symbole, Bereichs-Marker als Balken), Tooltip mit Name/Notiz, Drag zum Verschieben, Doppelklick öffnet den Bearbeiten-Dialog, Kontextmenü (Farbe, Löschen). Clip-Marker als kleine Kerben auf dem Clip. Marker auch im Quellmonitor-Lineal für das geladene Asset.
4. Marker-Panel (neues Panel im PanelHost): Liste aller Marker der Sequenz mit Farbe, Timecode, Name, Notiz — Klick springt den Playhead, Inline-Editing, Filter nach Farbe.
5. Tests: Marker-Persistenz-Roundtrip, Clip-Marker-Verhalten bei Trim/Split/Move, Navigation (nächster/vorheriger inkl. Randfälle).

Qualitätsanspruch: Marker sind ein Kern-Workflow für Review und Musikschnitt — das Setzen während laufender Wiedergabe muss latenzfrei sein, die Navigation präzise. Das Ergebnis muss sich an Premiere Pro messen lassen.
```

## 7. Insert-Edit & Three-Point-Editing

Einfügen ist überall Overwrite-Semantik; es gibt kein Insert mit Ripple und
keinen Quelle-In/Out → Timeline-Workflow. Spur-Targeting und Sync-Lock fehlen
ebenfalls.

```text
Implementiere Insert-Edit und Three-Point-Editing in Editron — auf absolut professionellem Niveau, wie in Adobe Premiere Pro und DaVinci Resolve, sodass professionelle Video-Produzenten das Tool produktiv nutzen können.

Ausgangslage: Einfügen in die Timeline hat überall Overwrite-Semantik (src/core/timeline.rs::insert_assets). Der Quellmonitor hat bereits lokale In/Out-Marken, aber es gibt keinen Weg, damit in die Timeline zu schneiden. Spur-Targeting und Sync-Lock existieren nicht.

Umfang:
1. Insert-Edit im TimelineStore: Einfügen mit Ripple — alle Clips ab dem Einfügepunkt auf den betroffenen Spuren rücken nach hinten (Clips, die der Einfügepunkt durchschneidet, werden gesplittet). Sync-Lock pro Spur (Header-Toggle): Spuren mit Sync-Lock rippeln mit, auch wenn sie kein neues Material bekommen — so bleibt der Mehrspur-Sync erhalten (Premiere-Semantik exakt nachbilden).
2. Spur-Targeting: Source-Patching im Timeline-Header — welche Quell-Kanäle (V1, A1, …) auf welche Timeline-Spuren gehen (klickbare Patch-Buttons links im Spur-Header), plus Spur-Targeting für playheadbezogene Operationen. Persistiert im Projekt.
3. Three-Point-Editing: Aus Quellmonitor-In/Out (bzw. ganzem Clip ohne Marken) plus Playhead/Sequenz-In/Out das Edit berechnen — die klassischen Fälle: Quelle In+Out + Ziel-Punkt; Quelle In + Ziel In+Out (Out der Quelle ergibt sich); Vier-Punkt mit Warnung. Commands „Einfügen“ (Komma) und „Überschreiben“ (Punkt) wie Premiere, in allen Keymap-Presets; Buttons im Quellmonitor (Insert/Overwrite-Icons).
4. Ergänzende Profi-Edits, die darauf aufbauen: Lift (In/Out-Bereich leeren, Lücke bleibt) und Extract (Bereich entfernen mit Ripple, Mod+X-Semantik wie Premiere — Lift „;“ / Extract „'“), sowie Match Frame (F: Frame unter dem Playhead im Quellmonitor mit exakter Medienzeit öffnen).
5. A/V-Verknüpfung respektieren: verknüpfte Paare landen gemäß Patching zusammen, Insert rippt Video- und Audiospuren konsistent. Undo: eine Geste = ein History-Eintrag.
6. Tests: Insert mit Split am Einfügepunkt, Sync-Lock-Matrix (mit/ohne Lock je Spur), Three-Point-Fallunterscheidung, Lift/Extract, Roundtrip.

Qualitätsanspruch: Das ist die Kern-Schnittmechanik, an der Profis ein NLE messen — die Semantik muss Premiere Pro exakt entsprechen (inklusive der Randfälle mit gesperrten und sync-gelockten Spuren), tastaturgetrieben und ohne Maus vollständig bedienbar.
```

## 8. Audio-Effekte & Spur-Automation

Keine Audio-Effekte (EQ, Kompressor, De-Noise), keine Spur-Automation — nur
Clip-Lautstärke-Keyframes und statisches Spur-Gain/Pan im Mixer.

```text
Implementiere Audio-Effekte und Spur-Automation in Editron — auf absolut professionellem Niveau, wie der Essential-Sound-/Mixer-Workflow in Adobe Premiere Pro bzw. die Fairlight-Grundlagen in DaVinci Resolve, sodass professionelle Video-Produzenten das Tool produktiv nutzen können.

Ausgangslage: Der Audio-Mixdown (src/core/player.rs) summiert Clips mit Spur-Gain/Pan, Clip-Gain inkl. Lautstärke-Keyframes und Master-Fader; der Mixer (src/panels/audio_mixer.rs) zeigt echte Pegel. Es gibt keine Audio-Effekte und keine Spur-Automation. Eine Effekt-Pipeline für Video ist parallel in Arbeit — falls deren Datenmodell (Effekte als Liste am Clip) schon existiert, daran ausrichten.

Umfang:
1. Audio-Effekt-Kette pro Spur und pro Clip im Timeline-Modell: geordnete Liste von Effekt-Instanzen mit Parametern, persistiert im .etron-Format, undo-fähig. Erste Effekte, alle in Rust direkt im Mixdown-Pfad als DSP implementiert (blockweise f32-Verarbeitung, keine ffmpeg-Roundtrips im Echtzeitpfad): parametrischer EQ (mind. 4 Bänder: Low-Shelf, 2× Bell, High-Shelf, mit Frequenz/Gain/Q), Kompressor (Threshold/Ratio/Attack/Release/Makeup, mit Gain-Reduction-Anzeige), Limiter (Master-tauglich), Hoch-/Tiefpass, Verstärkung.
2. Dieselbe DSP-Verarbeitung im Export (Phase A des Render-Workers in src/core/export.rs) — Wiedergabe und Export müssen identisch klingen (gemeinsamer DSP-Code, ein Modul, von beiden Pfaden genutzt).
3. Spur-Automation: Lautstärke und Pan pro Spur als Keyframe-Kurven über die Sequenzzeit (nicht Medienzeit) — Modell analog AnimatedParam aus src/core/animation.rs. Darstellung und Bearbeitung als Gummiband-Kurve direkt auf der Audiospur in der Timeline (Punkte setzen per Mod+Klick, ziehen, löschen — wie Premiere) und im Mixer (Fader-Bewegungen bei aktivierter Automations-Aufzeichnung optional als Phase 2).
4. UI: Effekt-Kette im Panel Effekteinstellungen für den ausgewählten Clip bzw. eine Spur-Ansicht (Reihenfolge per Drag änderbar, Bypass pro Effekt, Reset). EQ mit Kurven-Visualisierung, Kompressor mit Pegel-/GR-Metern. Alles über Commands registriert.
5. Tests: DSP-Korrektheit (EQ-Frequenzgang an Stützstellen, Kompressor-Kennlinie, Limiter-Ceiling), Parität Wiedergabe/Export (gerenderte Datei gegen Mixdown-Referenz), Automation-Interpolation, Projekt-Roundtrip.

Qualitätsanspruch: Klangneutralität und Stabilität sind Pflicht — keine Knackser an Blockgrenzen, keine Zipper-Noise bei Parameteränderungen (Parameter-Smoothing), Latenz im Echtzeitpfad unverändert. Professionelle Produzenten müssen damit einen sendefähigen Mix bauen können.
```
